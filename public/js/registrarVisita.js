...
function crearTabla(id,grupos, cantidades, body){
    ...
    +"<tr>"
    +"<th class='hidden-phone' width='30%'>Grupo Alimenticio</th>"
    ...
    for(var i= 0 ; i< grupos.length; i++){
        ...
        var alimen = x.insertCell(2);
        var agregar = x.insertCell(3);


        grupo.innerHTML = grupos[i];
        cant.innerHTML = "<input class='form-control' type='number' id='txt"+tabla.id+"-"+i+"'/>";
        agregar.innerHTML = "<a id='btn"+i+tabla.id+"' href='#agregarAlimentos' data-toggle='modal' ><i class='fa fa-pencil'/></a>";
        for(var j=0; j<alimentos.length ;j++){
         alimen.innerHTML += alimentos[j] +', ';
    }
